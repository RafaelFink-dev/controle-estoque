...
type
  TformConsultarProdutos = class(TForm)
    ...
    edPesquisa: TEdit;
    cbFiltros: TComboBox;
    fbProdutosPRECO_COMPRA: TFMTBCDField;
    fbProdutosPRECO_VENDA: TFMTBCDField;
...
begin
  if not fbProdutos.IsEmpty then
  begin
    try
      telaCadastro := TformCadastrasProdutos.Create(Self);
      telaCadastro.Caption := 'ALTERAÇÃO DE PRODUTOS';
      ...
      // Instanciamos um TMemoryStream
      msFoto := TMemoryStream.Create;
      // Carregamos a imagem para o TMemoryStream
      TBlobField(fbProdutos.FieldByName('IMAGEM')).SaveToStream(msFoto);
      // Seta a posição 0 do TMemoryStream
      msFoto.Position := 0;
      // Carregamos a imagem para o TImage da tela
  ...
  begin
    ShowMessage('Nenhum produto para alterar!');
  end;
end;
